      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {mod.materials.length > 0 && (
            <div className="flex gap-2 flex-wrap mt-2">
              {mod.materials.map((m) => (
                <a key={m.id} href={m.fileUrl} target="_blank" rel="noopener" className="text-xs flex items-center gap-1 text-brand-accent hover:underline">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Switch
            ...
            onCheckedChange={(v) => onTogglePublish(mod.id, v)}
          />
...
export default function ModulosPage() {
  ...
  useEffect(() => {
    fetch("/api/supervisor/modules").then((r) => r.json()).then(setModules)
  }, [])
  ...
  async function handleDragEnd(event: DragEndEvent) {
    ...
    await Promise.all(
      reordered.map((m, i) =>
        fetch(`/api/supervisor/modules/${m.id}`, {
          method: "PUT",
  ...
  async function handleSave() {
    ...
    const res = await fetch(
      isNew ? "/api/supervisor/modules" : `/api/supervisor/modules/${editingModule.id}`,
      {
  ...
  async function handleDelete(id: string) {
    ...
    await fetch(`/api/supervisor/modules/${id}`, { method: "DELETE" })
  ...
  async function handleTogglePublish(id: string, published: boolean) {
    ...
    const res = await fetch(`/api/supervisor/modules/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  async function handleUpload(moduleId: string, file: File, title: string) {
    ...
    const res = await fetch(`/api/supervisor/modules/${moduleId}/materials`, {
      method: "POST",
      body: formData,
    })
    ...
    <div className="space-y-6">
      ...
      {modules.length === 0 && (
        <p className="text-center text-muted-foreground py-12">
          No hay módulos aún. Creá el primero.
        </p>
      )}
