...
export default function TableroIdeasPage() {
  ...
  const [saberItems, setSaberItems] = useState<RankItem[]>([]) // entry order (passion cards)
  ...
  const [saberRank, setSaberRank] = useState<RankItem[]>([]) // passionate subset, ranked
  const [quererRank, setQuererRank] = useState<RankItem[]>([]) // all, ranked
  const [sonarRank, setSonarRank] = useState<RankItem[]>([]) // all, ranked
  const [saving, setSaving] = useState(false)

  // [T1] Block retake on mount.
  useEffect(() => {
  ...
  // step-1 string columns + brainstorming, so old-format drafts ({ saber,
  // querer, sonar, brainstorming }) load cleanly — any missing field falls back
  // to a default and the step-2 ranking state is rebuilt when the user advances.
  useEffect(() => {
  ...
  // Auto-save the step-1 inputs every 30s (the laborious part to protect).
